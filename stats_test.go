...
// `pattern "/metrics" conflicts`. This happens whenever an embedder (e.g.
// dnclient) tears down and re-runs Main()/Control.Start() in-process — for
// example on an overlay-network change. Each start must get its own HTTP mux.
